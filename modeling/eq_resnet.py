...
if hasattr(model, 'gspace_orientation'):
    gspace, fixparams, cutoff_dilation = set_gspace(model.gspace_orientation, model.gspace_flip, model.cutoff_dilation)
else:
    import joint_model
    gspace, fixparams, cutoff_dilation = set_gspace(joint_model.gspace_orientation, joint_model.gspace_flip, joint_model.cutoff_dilation)
...
def conv3x3(inplanes, out_planes, stride=1, padding=1, groups=1, dilation=1):
    ...
    out_type = FIELD_TYPE['regular'](gspace, out_planes)
    if cutoff_dilation and dilation > 1:
...
def convnxn(inplanes, outplanes, kernel_size=3, stride=1, padding=0, groups=1, bias=False, dilation=1,  is_backbone=True, to_trivial=False):
    in_type = FIELD_TYPE['regular'](gspace, inplanes)
    if to_trivial:
        out_type = FIELD_TYPE['trivial'](gspace, outplanes, fixparams=False)
    else:
...
class ReResNet(BaseBackbone):
    ...
    def export(self):
        ...
        for i, layer_name in enumerate(self.res_layers):
            res_layer = getattr(self, layer_name)
            for subname, submodule in res_layer._modules.items():
                if isinstance(submodule, enn.EquivariantModule):
                    # print(subname, "--->", submodule)
                    submodule = submodule.export()
        return self
